...
`quote-ingester` 是一个常驻运行的 K 线采集服务。它从 Azverse 行情接口拉取 `1m` K 线数据，保存到 Postgres，并在本地自动聚合生成 `5m`、`30m` K 线，供后续分析服务读取。

## 功能概览

- 从 `https://app.azverse.xyz/sapi/v1/quote/public/kline` 拉取行情数据。
- 默认采集 `btc_usdt` 的 `1m` K 线。
- 启动时回填最近 30 天数据。
...
- `api.rs`：调用 Azverse K 线接口，并把接口字段 `t/o/h/l/c/q/v` 转成内部模型。
...
- `quote_api.source`：数据来源标识，默认是 `azverse`。
- `quote_api.base_url`：行情接口域名。
- `quote_api.timeout_seconds`：单次 HTTP 请求超时时间。
- `markets.symbol`：交易对，例如 `btc_usdt`。
...
```text
azverse / btc_usdt / 1m  / 2026-06-21 10:00:00
azverse / btc_usdt / 5m  / 2026-06-21 10:00:00
azverse / btc_usdt / 30m / 2026-06-21 10:00:00
```
...
- `low_price`：最低价
- `close_price`：收盘价
- `base_volume`：基础币成交量，对应接口里的 `q`
- `quote_volume`：计价币成交额，对应接口里的 `v`
